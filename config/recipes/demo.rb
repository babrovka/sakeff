task :demo do
  ...
  task :import_permissions do
     run %Q{cd #{latest_release} && RAILS_ENV=demo bundle exec rake excel:permissions}
  end

  task :copy_and_import_units do
     run %Q{cd #{latest_release} && cp source3d/objectTree.xls db/excel/units.xls && RAILS_ENV=demo bundle exec rake excel:units}
  end
  
  task :eve_states do
     run %Q{cd #{latest_release} && RAILS_ENV=demo bundle exec rake templates:states}
  end
  ...
  namespace(:thin) do
    task :stop do
      run %Q{cd #{latest_release} && RAILS_ENV=demo bundle exec thin stop -C #{shared_path}/sakedev.yml}
     end

    task :start do
      run %Q{cd #{latest_release} && RAILS_ENV=demo bundle exec thin start -C #{shared_path}/sakedev.yml}
    end
  ...
  namespace :private_pub do
    ...
    task :start do
      # old command
      run %Q{cd #{latest_release} && RAILS_ENV=demo bundle exec thin start -C #{shared_path}/private_pub.yml }
      # run %Q{cd #{latest_release} && RAILS_ENV=demo bundle exec thin -C #{shared_path}/private_pub.yml start }
    ...
    task :stop do
      ...
      run %Q{cd #{latest_release} && RAILS_ENV=demo bundle exec thin -C #{shared_path}/private_pub.yml stop}
    end
  ...
  namespace(:populate) do
    task :data do
      run %Q{cd #{latest_release} && bundle exec rake db:seed RAILS_ENV=demo}
    end
  end
